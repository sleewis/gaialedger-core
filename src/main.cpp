...
#if defined(NDEBUG)
# error "Gaia cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Gaia Signed Message:\n";
...
CAmount GetProofOfWorkSubsidy()
{
    ...
    if (nBlockHeight == 1) {
     return 6666666 * COIN; // all swap STK & ENRG
    }
    if (nBlockHeight != 1) {
    return 1 * COIN;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Gaia also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // Gaia also requires low S in sigs
